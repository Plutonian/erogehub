...
class DownloadBrandHandler extends DefaultMessageHandler[Integer] {
  ...
  override def process(message: Message[Integer], msgQueue: MessageQueueProxy[Message[_]]): Unit = {
    ...
    try GetChu.BrandService.download(id)
      ...
      case e: Exception =>
        ...
        msgQueue.offer(new Message[Integer](99, id))
        e.printStackTrace()
    }
    msgQueue.offer(new Message[Integer](MesType.Brand, id))
  }
}