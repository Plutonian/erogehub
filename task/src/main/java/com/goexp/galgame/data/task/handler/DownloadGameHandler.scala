...
class DownloadGameHandler extends DefaultMessageHandler[Integer] {
  ...
  override def process(message: Message[Integer], msgQueue: MessageQueueProxy[Message[_]]): Unit = {
    ...
    try GetChu.GameService.download(gid)
      ...
      case e: Exception =>
        ...
        msgQueue.offer(new Message[Integer](MesType.NEED_DOWN_GAME, gid))
        e.printStackTrace()
    }
    msgQueue.offer(new Message[Integer](MesType.Game, gid))
  }
}