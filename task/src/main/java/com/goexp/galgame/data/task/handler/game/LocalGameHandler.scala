...
class LocalGameHandler extends DefaultMessageHandler[Integer] {
  ...
  override def process(message: Message[Integer], msgQueue: MessageQueueProxy[Message[_]]): Unit = {
    ...
    val bytes = util.Map.entry(gid, Objects.requireNonNull(getContent(gid), gid.toString))
    msgQueue.offer(new Message[util.Map.Entry[Integer, Array[Byte]]](MesType.ContentBytes, bytes))
  }

  private def getContent(id: Int) = try {
